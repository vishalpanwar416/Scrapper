...
import ProductValidator, { ValidationResult, RawProduct, CleanedProduct } from './productValidator.js';
import DataSanitizer, { SanitizationResult } from './dataSanitizer.js';
...
export interface BatchPipelineResult {
  ...
  failedProducts: { product: any; reason: string }[];
  summary: {
    successRate: number;
...
export class ValidationPipeline {
   ...
   */
  static processProduct(rawProduct: RawProduct): PipelineResult | null {
    ...
    const sanitizedProduct: RawProduct = {
   ...
   */
  static processBatch(rawProducts: RawProduct[]): BatchPipelineResult {
    const results: PipelineResult[] = [];
    const failedProducts: { product: RawProduct; reason: string }[] = [];
    let totalWarnings = 0;
    let totalErrors = 0;
   ...
   * Get pipeline report
   */
  static generateReport(batchResult: BatchPipelineResult): string {
    const lines = [
