...
export class DataSanitizer {
   ...
   */
  static sanitizeTitle(title: any): { clean: string; changed: boolean } {
    if (!title) return { clean: '', changed: true };
   ...
   */
  static sanitizeUrl(url: any): { clean: string; changed: boolean } {
    if (!url) return { clean: '', changed: true };
   ...
   * Sanitize price
   */
  static sanitizePrice(price: any): { clean: number; changed: boolean } {
   ...
   */
  static sanitizeDescription(description: any): { clean: string; changed: boolean } {
    if (!description) return { clean: '', changed: false };
   ...
   * Sanitize image URL
   */
  static sanitizeImageUrl(url: any): { clean: string; changed: boolean } {
    if (!url) return { clean: '', changed: false };
   ...
   */
  static sanitizeColors(colors: any): { clean: string[]; changed: boolean } {
    if (!Array.isArray(colors)) return { clean: [], changed: true };
   ...
   */
  static sanitizeSizes(sizes: any): { clean: string[]; changed: boolean } {
    if (!Array.isArray(sizes)) return { clean: [], changed: true };
   ...
   */
  private static stripDangerousContent(str: string): string {
    // Remove script tags
   ...
   */
  private static standardizeSizeFormat(size: string): string {
    // Common replacements
  ...
  static sanitizeProduct(product: any): SanitizationResult & { valid: boolean } {
    ...
    if (originalPriceChanged && product.originalPrice) changes.push('originalPrice');
    ...
    if (imageChanged) changes.push('imageUrl');
    ...
    if (sizesChanged) changes.push('sizes');
