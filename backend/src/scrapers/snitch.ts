...
export async function scrapeSnitch(
  websiteId: string,
  ...
  try {
        ...
        await Promise.race([
          page.waitForFunction(() => {
            const count = document.querySelectorAll(
              'a[href*="/products/"], [data-testid*="product"], [class*="product-card"]'
            ).length;
            return count > 3;
          }, { timeout: 20000 }),
    ...
    updateProgress_(
      ScrapeStage.COMPLETED,
      `Successfully scraped ${result.itemsScraped} new products and updated ${result.itemsUpdated} existing products!`,
      100
    );
 ...
 */
async function extractProductsFromPage(page: any): Promise<ScrapedProduct[]> {
  try {
    return await page.evaluate(() => {
      ...
      productElements.forEach((el) => {
        try {
          ...
          if (href.includes('?') && !href.includes('product')) return;
          ...
          if (!title || title.length < 3 || title.length > 250) return;

          // Filter non-product titles
          const titleLower = title.toLowerCase();
          if (nonProductKeywords.some(k => titleLower.includes(k))) return;
