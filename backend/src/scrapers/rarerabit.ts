...
export async function scrapeRareRabit(websiteId: string, progressCallback?: ProgressCallback): Promise<{itemsScraped:number;itemsUpdated:number;status:string;error?:string}> {
  let browser = null;
  ...
  try {
    updateProgress_('starting', 'Initializing Rare Rabbit scraper...', 10);
    browser = await launchBrowser();
    const page = await browser.newPage();
    page.setDefaultNavigationTimeout(30000);
    page.setDefaultTimeout(30000);

    // Go to Rare Rabbit's product listing page
    const urls = [
      'https://thehouseofrare.com/collections/new-arrivals',
      'https://thehouseofrare.com/collections/mens',
      'https://thehouseofrare.com/collections/womens',
    ];
    ...
    for (const url of urls) {
      try {
        console.log(`[Rare Rabbit] Scraping: ${url}`);
        await page.goto(url, { waitUntil: 'networkidle2', timeout: 30000 });

        // Wait for product grid to load
        await page.waitForSelector('.product-item, [data-product], .product-card', { timeout: 5000 }).catch(() => {});

        // Extract products
        const pageProducts = await page.evaluate(() => {
          const items: any[] = [];
          const productElements = document.querySelectorAll('.product-item, [data-product], .product-card, .product-tile');
              ...
              if (titleEl && priceEl) {
                ...
                if (title && url) {
                  items.push({
                    title,
                    price,
                    url: url.startsWith('http') ? url : `https://thehouseofrare.com${url}`,
                    imageUrl: imageUrl.startsWith('http') ? imageUrl : `https://thehouseofrare.com${imageUrl}`,
                  });
                }
              }
            } catch (e) {
        ...
        console.error(`[Rare Rabbit] Error scraping ${url}:`, error.message);
      }
    }
    ...
    if (productsToSave.length === 0) {
      console.log('[Rare Rabbit] No products found via Puppeteer, using demo data');
      productsToSave = await generateDemoProducts('rarerabbit');
    }
    ...
    console.error('[Rare Rabbit] Scraping failed:', error);
