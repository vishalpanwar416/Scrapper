...
export async function saveProductsToDatabase(
  ...
  const productsToUpdate: Array<{ existing: any; data: any }> = [];

  // Check which products exist (optimized single query)
  const existingProducts = await prisma.product.findMany({
    where: {
      websiteId,
      url: { in: batchResult.results.map((r: any) => r.data?.url).filter(Boolean) },
    },
  });
      ...
      await prisma.$transaction(async (tx) => {
        for (const { existing, data } of productsToUpdate) {
          ...
          if (data.colors && Array.isArray(data.colors) && data.colors.length > 0) {
            ...
            await tx.color.createMany({
              data: data.colors.map((colorName: any) => ({
