...
import { initializeProgress, updateProgress, clearProgress } from './progressTracker.js';
...
export class AutoScraper {
   ...
   */
  static async triggerScrapSync(websiteId: string): Promise<any> {
    try {
   ...
   */
  private static async scrapeWebsite(websiteId: string): Promise<any> {
    try {
      // Get website details
      ...
      let result;
      if (type === 'specific') {
        result = await (scraper as any)(websiteId);
      } else {
        result = await (scraper as any)(websiteId, website.url);
      }
   ...
   */
  static async scrapeMultiple(websiteIds: string[]): Promise<any[]> {
    const results = [];
   ...
   */
  static async scrapeAll(): Promise<any[]> {
    try {
