 ...
 */
router.get('/logs/:websiteId', asyncHandler(async (req: Request, res: Response) => {
 ...
 */
router.get('/progress/:websiteId', asyncHandler(async (req: Request, res: Response) => {
  const { websiteId } = req.params;
 ...
 */
router.get('/scrapers/list', (req: Request, res: Response) => {
  try {
 ...
 */
router.get('/scrapers/check/:websiteName', (req: Request, res: Response) => {
  try {
 ...
 */
router.post('/start/:websiteName', asyncHandler(async (req: Request, res: Response) => {
  const { websiteName } = req.params;
  ...
  try {
    ...
    if (type === 'specific') {
      result = await (scraper as any)(website.id);
    } else {
      result = await (scraper as any)(website.id, website.url);
    }
    ...
    throw new CustomError(
      ...
      500,
      'SCRAPE_ERROR',
      { website: websiteName, error: error?.message || 'Unknown error' }
    );
  } finally {
