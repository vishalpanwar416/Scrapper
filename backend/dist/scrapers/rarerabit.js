...
exports.scrapeRareRabit = scrapeRareRabit;
const prisma_1 = __importDefault(require("../database/prisma"));
const puppeteer_1 = __importDefault(require("puppeteer"));
async function scrapeRareRabit(websiteId) {
    const products = [];
    let itemsScraped = 0;
    let itemsUpdated = 0;
    let browser = null;
    try {
        const baseUrl = 'https://www.rarerabit.com';
        console.log(`Starting scrape for RareRabit (${websiteId})`);
        // Launch headless browser
        browser = await puppeteer_1.default.launch({
            headless: true,
            args: ['--no-sandbox', '--disable-setuid-sandbox']
        });
        // Example category URLs - adjust based on actual site structure
        const categoryUrls = [
            `${baseUrl}`,
            `${baseUrl}/collections/mens-clothing`,
            `${baseUrl}/collections/womens-clothing`,
        ...
        for (const categoryUrl of categoryUrls) {
            try {
                ...
                await page.waitForSelector('a[href*="/products/"], .product-card', {
                    timeout: 5000
                }).catch(() => {
                ...
                const pageProducts = await page.evaluate(() => {
                    const products = [];
                    const productElements = document.querySelectorAll('a[href*="/products/"], .product-card, .product-item, [data-product-id]');
                    productElements.forEach((el) => {
                        try {
                            ...
                            const titleEl = el.querySelector('h2, h3, .product-title, .product-name, [data-product-title]');
                ...
                console.error(`Error scraping RareRabit category:`, categoryError);
            }
        }
        ...
        console.error('Error scraping RareRabit:', error);
        return {
            itemsScraped,
            itemsUpdated,
            status: 'failed',
...
//# sourceMappingURL=rarerabit.js.map