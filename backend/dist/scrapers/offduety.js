...
exports.scrapeOffDuety = scrapeOffDuety;
const prisma_1 = __importDefault(require("../database/prisma"));
const puppeteer_1 = __importDefault(require("puppeteer"));
async function scrapeOffDuety(websiteId) {
    const products = [];
    let itemsScraped = 0;
    let itemsUpdated = 0;
    let browser = null;
    try {
        const baseUrl = 'https://offduty.in';
        console.log(`Starting scrape for OffDuety (${websiteId})`);
        // Launch headless browser
        browser = await puppeteer_1.default.launch({
            headless: true,
            args: ['--no-sandbox', '--disable-setuid-sandbox']
        });
        // Category URLs for OffDuety India
        const categoryUrls = [
            `${baseUrl}/collections/men`,
            `${baseUrl}/collections/women`,
            `${baseUrl}/collections/footwear`,
        ...
        for (const categoryUrl of categoryUrls) {
            try {
                ...
                await page.waitForSelector('a[href*="/products/"], .product, .product-tile', {
                    timeout: 5000
                }).catch(() => {
                ...
                const pageProducts = await page.evaluate(() => {
                    const products = [];
                    const productElements = document.querySelectorAll('a[href*="/products/"], .product, .product-tile, [data-product]');
                    productElements.forEach((el) => {
                        try {
                            ...
                            const titleEl = el.querySelector('h2, h3, .title, .product-name, [data-product-title]');
                ...
                console.error(`Error scraping OffDuety category:`, categoryError);
            }
        }
        ...
        console.error('Error scraping OffDuety:', error);
        return {
            itemsScraped,
            itemsUpdated,
            status: 'failed',
...
//# sourceMappingURL=offduety.js.map