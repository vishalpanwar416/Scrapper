 ...
 */
router.get('/:websiteName', asyncHandler(async (req, res) => {
    const { websiteName } = req.params;
    const scraper = await scraperFileManager.getScraperContent(websiteName);
    if (!scraper) {
        throw new CustomError(`Scraper not found for website: ${websiteName}`, 404, 'SCRAPER_NOT_FOUND');
    }
    res.json({
        success: true,
        websiteName,
 ...
 */
router.put('/:websiteName', asyncHandler(async (req, res) => {
    const { websiteName } = req.params;
    const { content } = req.body;
    if (!content || typeof content !== 'string') {
        throw new CustomError('Content is required and must be a string', 400, 'INVALID_CONTENT');
    }
    if (content.length > 100000) {
        throw new CustomError('Scraper file too large (max 100KB)', 413, 'FILE_TOO_LARGE');
    }
    try {
        ...
        throw new CustomError(`Failed to update scraper: ${error.message}`, 500, 'SCRAPER_UPDATE_ERROR');
    }
}));
/**
 * Get scraper syntax with examples
 */
router.get('/:websiteName/syntax', asyncHandler(async (req, res) => {
    const { websiteName } = req.params;
 ...
 */
router.post('/:websiteName/validate', asyncHandler(async (req, res) => {
    const { websiteName } = req.params;
    const { content } = req.body;
    if (!content) {
 ...
 */
router.post('/:websiteName/test', asyncHandler(async (req, res) => {
    const { websiteName } = req.params;
    const { url } = req.body;
    if (!url) {
 ...
 */
router.post('/template/:websiteName', asyncHandler(async (req, res) => {
    const { websiteName } = req.params;
    const { url } = req.body;
    if (!url) {
