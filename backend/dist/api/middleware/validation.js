...
export const validateBody = (schema) => {
    return (req, res, next) => {
        ...
        if (Object.keys(errors).length > 0) {
            return next(new CustomError('Validation failed', 400, 'VALIDATION_ERROR', errors));
        }
        next();
    };
};
 ...
 */
export const validate = (bodySchema, querySchema, paramsSchema) => {
    return (req, res, next) => {
        const errors = {};
        // Validate body
        if (bodySchema) {
